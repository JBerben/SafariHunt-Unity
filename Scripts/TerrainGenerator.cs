...
public class TerrainGenerator : MonoBehaviour 
{
	...
	void Start() 
	{
		...
		for(int x = 0; x < m_tilesX; x++)
		{
			...
			{
				// The main part of drawing terrain
				// This function enters all of the heights
				// of mountains and rolling hills/flatland into
				// the heightmap array
	...
	void FillHeights(float[,] htmap, int tileX, int tileZ)
	{
		...
		for(int x = 0; x < m_heightMapSize; x++)
		{
			for(int z = 0; z < m_heightMapSize; z++)
			{
				...
				//Mountains are only drawn if the value is > 0 giving plains a chance to be drawn
				...
				//adds newly calculated plans & mountains back into heightmap array
