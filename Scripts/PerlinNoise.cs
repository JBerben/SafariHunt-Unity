...
{
	const int B = 256; // Determines the size of the Perlin Array
	int[] m_perm = new int[B+B];

	public PerlinNoise(int seed)
	{
		UnityEngine.Random.seed = seed; // Produces random number

		int i, j, k; // k: Gets set to current slot in m_perm array
		             // j: Is a number anywhere from 0 - B


		for (i = 0 ; i < B ; i++)  // Temporarily fills up
		{                          // the array with data
			m_perm[i] = i;         // eg. slot m_perm[1] = 1
		}

		while (--i != 0) 
		{
			k = m_perm[i];
			j = UnityEngine.Random.Range(0, B); // Sets j to any no. from 0 - B
			m_perm[i] = m_perm[j];
			m_perm[j] = k;
		}
